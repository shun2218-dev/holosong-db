  ...
  return (
    <div className="max-w-4xl mx-auto py-10">
      <div className="grid md:grid-cols-2 gap-10">
        ...
        {/* 右側：楽曲リスト (新規追加部分) */}
        <div>
          <h2 className="text-2xl font-bold mb-6">登録済み一覧 ({allSongs.length})</h2>
          <div className="space-y-4 max-h-[800px] overflow-y-auto pr-2">
            {allSongs.map((song) => (
              <div key={song.id} className="flex items-center gap-4 bg-white p-3 rounded-lg border border-gray-200 shadow-sm">
                {/* 画像 */}
                <div className="relative w-16 h-16 bg-gray-100 rounded overflow-hidden flex-shrink-0">
                  {song.jacketUrl && (
                    <Image src={song.jacketUrl} alt={song.title} fill className="object-cover" />
                  )}
                </div>
