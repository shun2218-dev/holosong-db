    ...
    <div>
      ...
      <div className="flex flex-col md:flex-row items-center gap-8 bg-white p-8 rounded-xl shadow-sm border border-gray-200 mb-10">
        <div className="relative w-32 h-32 md:w-40 md:h-40 rounded-full overflow-hidden bg-gray-100 border-2 border-gray-100 shadow-inner">
          {talentData.imageUrl && (
             <Image src={talentData.imageUrl} alt={talentData.name} fill className="object-cover" />
          )}
        </div>
        <div className="text-center md:text-left">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">{talentData.name}</h1>
          <p className="text-lg text-gray-500 mb-4">{talentData.generation}</p>
          <div className="text-sm text-gray-400">
             Participated in <span className="font-bold text-gray-900">{songList.length}</span> songs
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {songList.map((song) => (
            ...
            >
              ...
              <h3 className="text-lg font-bold mb-2 group-hover:text-blue-600 transition-colors">{song.title}</h3>
              <p className="text-sm text-gray-500 mb-3">{song.releaseDate}</p>
              
              {/* 一緒に歌っているタレント表示 */}
              <div className="flex flex-wrap gap-1">
                 {song.talents.map((t) => (
                   <span key={t.talentId} className={`text-xs px-2 py-1 rounded-full ${t.talentId === talentData.id ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600'}`}>
                     {t.talent.name}
                   </span>
                 ))}
              </div>
